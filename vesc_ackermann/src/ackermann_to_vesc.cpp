...
AckermannToVesc::AckermannToVesc(ros::NodeHandle nh, ros::NodeHandle private_nh)
{
  ...
  // create publishers to focbox electric-RPM (speed) and servo commands
  ...
  ackermann_sub_ = nh.subscribe("ackermann_cmd", 10, &AckermannToVesc::ackermannCmdCB, this);
}

typedef ackermann_msgs::AckermannDriveStamped::ConstPtr AckermannMsgPtr;
void AckermannToVesc::ackermannCmdCB(const AckermannMsgPtr& cmd)
{
  // calc focbox electric RPM (speed)
