  ...
  use_servo_cmd_(true), publish_tf_(false), x_(0.0), y_(0.0), yaw_(0.0)
{
  ...
  // subscribe to focbox state and. optionally, servo command
  vesc_state_sub_ = nh.subscribe("sensors/core", 10, &VescToOdom::vescStateCB, this);
  if (use_servo_cmd_) {
    servo_sub_ = nh.subscribe("sensors/servo_position_command", 10,
                              &VescToOdom::servoCmdCB, this);
  }
}

void VescToOdom::vescStateCB(const focbox_unity_msgs::FocboxUnityStateStamped::ConstPtr& state)
{
...
void VescToOdom::servoCmdCB(const std_msgs::Float64::ConstPtr& servo)
{
